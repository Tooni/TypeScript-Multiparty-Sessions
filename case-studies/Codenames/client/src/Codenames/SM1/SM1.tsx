...
export default class Session extends React.Component<Props, Partial<Transport>> {
    ...
    render() {
        ...
        return ws === undefined
            ? this.props.waiting
            : <SM1 ws={ws} {...this.props} />;
    }

}

class SM1 extends React.Component<Props & Transport, ComponentState> {
    ...
    constructor(props: Props & Transport) {
        ...
        this.messageQueue = {
            [Roles.Peers.FO1]: [], [Roles.Peers.Svr]: [], [Roles.Peers.SM2]: [], [Roles.Peers.FO2]: [],
        };
        this.handlerQueue = {
            [Roles.Peers.FO1]: [], [Roles.Peers.Svr]: [], [Roles.Peers.SM2]: [], [Roles.Peers.FO2]: [],
        };
    ...
    private sendMessage(role: Roles.Peers, label: string, payload: any, successor: State) {
        // Have to call toString() because one of SendState or ReceiveState could be empty
        const fromState = this.state.state.toString();
        const jsonString = JSON.stringify(Message.toChannel(role, label, payload, fromState));
